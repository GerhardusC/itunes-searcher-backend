...
const getResults = async (searchTerm, mediaType) => {
    ...
    if(searchTerm && mediaType) {
        try {
            let result = await fetch(`https://itunes.apple.com/search?term=${searchTerm}&media=${mediaType}`)
    ...
    if(searchTerm) {
        try {
            let result = await fetch(`https://itunes.apple.com/search?term=${searchTerm}`)
            searchResults = await result.json()
        } catch (err) {
            console.log(err)
        } finally {
